                ...
                <div class="card-body">
                    <form method="POST" action="{{ route("frontend.reviews.update", [$review->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" name="service_request_id" id="service_request_id" required>
                                @foreach($service_requests as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('service_request_id') ? old('service_request_id') : $review->service_request->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control" type="number" name="rating" id="rating" value="{{ old('rating', $review->rating) }}" step="1" required>
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="form-control" name="comment" id="comment" required>{{ old('comment', $review->comment) }}</textarea>
