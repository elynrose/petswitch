        ...
        <div class="col-md-12">
            @can('photo_update_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.photo-updates.create') }}">
                                    ...
                                    <tr data-entry-id="{{ $photoUpdate->id }}">
                                        ...
                                        <td>
                                            @can('photo_update_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.photo-updates.show', $photoUpdate->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('photo_update_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.photo-updates.edit', $photoUpdate->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('photo_update_delete')
                                                <form action="{{ route('frontend.photo-updates.destroy', $photoUpdate->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('photo_update_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.photo-updates.massDestroy') }}",
