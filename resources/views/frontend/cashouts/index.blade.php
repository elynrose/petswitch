        ...
        <div class="col-md-12">
            @can('cashout_create')
                <div style="margin-bottom: 10px;" class="row">
                    <div class="col-lg-12">
                        <a class="btn btn-success" href="{{ route('frontend.cashouts.create') }}">
                            ...
                            <tbody>
                                @foreach($cashouts as $key => $cashout)
                                    <tr data-entry-id="{{ $cashout->id }}">
                                        ...
                                        <td>
                                            @can('cashout_show')
                                                <a class="btn btn-xs btn-primary" href="{{ route('frontend.cashouts.show', $cashout->id) }}">
                                                    {{ trans('global.view') }}
                                                </a>
                                            @endcan

                                            @can('cashout_edit')
                                                <a class="btn btn-xs btn-info" href="{{ route('frontend.cashouts.edit', $cashout->id) }}">
                                                    {{ trans('global.edit') }}
                                                </a>
                                            @endcan

                                            @can('cashout_delete')
                                                <form action="{{ route('frontend.cashouts.destroy', $cashout->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('cashout_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('frontend.cashouts.massDestroy') }}",
