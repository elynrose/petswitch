    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pets.update", [$pet->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('breed') ? 'is-invalid' : '' }}" type="text" name="breed" id="breed" value="{{ old('breed', $pet->breed) }}" required>
                @if($errors->has('breed'))
                    <div class="invalid-feedback">
                        {{ $errors->first('breed') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('age') ? 'is-invalid' : '' }}" type="text" name="age" id="age" value="{{ old('age', $pet->age) }}" required>
                @if($errors->has('age'))
                    <div class="invalid-feedback">
                        {{ $errors->first('age') }}
                    </div>
            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Pet::GETS_ALONG_WITH_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('gets_along_with') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="gets_along_with_{{ $key }}" name="gets_along_with" value="{{ $key }}" {{ old('gets_along_with', $pet->gets_along_with) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('is_immunized') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="is_immunized" id="is_immunized" value="1" {{ $pet->is_immunized || old('is_immunized', 0) === 1 ? 'checked' : '' }}>
