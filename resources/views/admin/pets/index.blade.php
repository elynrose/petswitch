...
@can('pet_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.pets.create') }}">
                ...
                <tbody>
                    @foreach($pets as $key => $pet)
                        <tr data-entry-id="{{ $pet->id }}">
                            ...
                            <td>
                                @can('pet_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.pets.show', $pet->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('pet_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.pets.edit', $pet->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('pet_delete')
                                    <form action="{{ route('admin.pets.destroy', $pet->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('pet_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.pets.massDestroy') }}",
