    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pet-reviews.update", [$petReview->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('comment') ? 'is-invalid' : '' }}" name="comment" id="comment" required>{{ old('comment', $petReview->comment) }}</textarea>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rating') ? 'is-invalid' : '' }}" type="number" name="rating" id="rating" value="{{ old('rating', $petReview->rating) }}" step="1" required>
